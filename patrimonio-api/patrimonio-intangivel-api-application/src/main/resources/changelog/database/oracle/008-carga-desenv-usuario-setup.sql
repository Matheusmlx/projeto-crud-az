...
values (comum.seq_usuario.nextval, 'servidor', 'servidor@azi.com.br', '61486745210', 'Servidor', 0, '33233232', 'ATIVO', 1, 30, 'INTERNO');

insert into comum.tb_usuario (us_id, us_email, us_email_contato, us_doc_chave, us_nome, us_exibir_representacoes, us_telefone, us_situacao, id_id, pa_id, us_tipo_usuario)
values (comum.seq_usuario.nextval, 'consultor', 'consultor@azi.com.br', '93548959059', 'Consultor', 0, '33233232', 'ATIVO', 1, 30, 'INTERNO');

-- USUARIO SENHA
insert into hal.tb_usuario_hal (us_id, us_senha)
values ((select us_id from comum.tb_usuario where us_email = 'servidor'), 'UTr6YlcHzuQ=' );

insert into hal.tb_usuario_hal (us_id, us_senha)
values ((select us_id from comum.tb_usuario where us_email = 'consultor'), 'UTr6YlcHzuQ=' );

-- DOMINIO
insert into hal.tb_dominio (dm_id, dm_tipo_cliente, dm_id_cliente, us_id)
select hal.seq_dominio.nextval, 'ESTRUTURA_ORGANIZACIONAL', uo_id, (select us_id from comum.tb_usuario where us_email = 'admin')
from comum_siga.tb_unidade_organizacional
where uo_cod_hierarquia like '0002.%';

insert into hal.tb_dominio (dm_id, dm_tipo_cliente, dm_id_cliente, us_id)
select hal.seq_dominio.nextval, 'ESTRUTURA_ORGANIZACIONAL', uo_id, (select us_id from comum.tb_usuario where us_email = 'servidor')
from comum_siga.tb_unidade_organizacional
where uo_cod_hierarquia like '0002.%';

insert into hal.tb_dominio (dm_id, dm_tipo_cliente, dm_id_cliente, us_id)
select hal.seq_dominio.nextval, 'ESTRUTURA_ORGANIZACIONAL', uo_id, (select us_id from comum.tb_usuario where us_email = 'consultor')
from comum_siga.tb_unidade_organizacional
where uo_cod_hierarquia like '0002.%';
