<template>
    <v-container fluid>
        <div>
            <az-desfazer :show="showDesfazer" text='Documento removido!' @desfazer="desfazerExclusao"
                         @closeButtom="showDesfazer=false"/>
        </div>
                                ...
                                <v-col cols="12" md="12" class="mt-0 pt-0 ml-n3">
                                    <v-simple-table class="simple-table">
                                        ...
                                        <tr :key="documento.id" v-for="(documento,index) in documentos">
                                            ...
                                            <az-remove-button @remover="tratarEventoDeletarDocumento(documento)"
    ...
    export default {
        ...
        methods: {
            ...
            novoDocumento() {
                if (this.validaSeEstaEditando()) {
                    if (this.validarCadastrosFinalizados(this.docNovo)) {
                        ...
                        this.docNovo = {idPatrimonio: this.$route.params.id}
                    } else {
            ...
            async tratarEventoSalvarAnexo(anexo) {
                this.desabilitarLoadingGlobal()
                if (anexo.uriAnexo) {
                    anexo.uriAnexo = await this.$store.dispatch(actionTypes.PATRIMONIO.DOCUMENTO.DOCUMENTO_UPLOAD, anexo)
